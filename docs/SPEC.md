...
- **Dispatch does not install a database server, and defaults to bundled SQLite.** The database is either a
  SQLite file the service creates beside itself (the default on every platform - no server, no credentials,
  no extra service to patch) or a PostgreSQL, MariaDB/MySQL or SQL Server instance the operator already
  runs. The engine is chosen by the connection string, with `Database:Provider` to disambiguate the one
  case sniffing cannot resolve (`Server=...;Database=...;User Id=...` is valid for both SQL Server and
  MySQL). The schema is defined once in `DispatchDbContext` and rendered per engine by EF Core migrations.
  Anywhere this document assumes PostgreSQL specifically, read "the configured database". See
  [database.md](database.md). (Affects §6, §12.1, §15.)
- **Windows install is a WiX Burn bundle, not a WinForms wizard.** §15.2 describes a standalone .NET WinForms setup wizard; the actual installer is `DispatchSetup.exe`, a WiX Burn bundle (`bundle/Bundle.wxs`) chaining the Dispatch MSI. It no longer installs a database server: earlier releases chained an `InstallPostgres.exe` launcher that downloaded and silently installed PostgreSQL, which took 15-20 minutes and left the service behind after uninstall. There is no interactive multi-step UI and no certificate-generation step on Windows - automatic self-signed cert generation exists only on Linux via `install.sh --generate-cert`. (Affects §15.2, §17.2.)
