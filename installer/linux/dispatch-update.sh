...
jget() { grep -o "\"$1\"[ ]*:[ ]*\"[^\"]*\"" "$REQ" | head -1 | sed 's/.*:[ ]*"//; s/"$//'; }

VER="$(jget version)"
STAGED="$(jget stagedDir)"
fail() { echo "update: $1" >&2; status "Failed" "$1"; rm -f "$REQ"; exit 1; }

[ -n "$VER" ] && [ -n "$STAGED" ] || fail "apply.request is missing version/stagedDir"
...
# 1) Re-verify independently of the web service (fail-closed): the manifest signature (authenticity), then
#    the staged payload's sha256 against this arch's entry in the (now-trusted) signed manifest.
echo "update: verifying $VER ($ARCH)"
openssl dgst -sha256 -verify "$PUBKEY" -signature "$STAGED/manifest.json.sig" "$STAGED/manifest.json" >/dev/null 2>&1 \
  || fail "signature verification failed"
if command -v python3 >/dev/null 2>&1; then
  WANT="$(python3 -c "import json;print(json.load(open('$STAGED/manifest.json'))['artifacts']['$ARCH']['sha256'])" 2>/dev/null || true)"
  GOT="$(sha256sum "$STAGED/payload" | cut -d' ' -f1)"
  [ -n "$WANT" ] && [ "$WANT" = "$GOT" ] || fail "payload checksum mismatch for $ARCH"
else
  echo "update: python3 absent - skipping the payload-hash recheck (the service already verified it)"
fi
...
backup_db() {
  ...
  local conn; conn="$(python3 -c "import json;print(json.load(open('$DATA_DIR/appsettings.json'))['ConnectionStrings']['DispatchLog'])" 2>/dev/null || true)"
  [ -n "$conn" ] || return 0
...
tar -xzf "$STAGED/payload" -C "$NEWDIR" || fail "could not extract the payload"
